  ...
  return (
    ...
    <div className='relative py-4 px-6 md:px-8 bg-white'>
        <div className='flex flex-col md:flex-row items-start gap-6'>
            ...
            <div className='upi-profile w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    <div>
                        ...
                        <p className='text-gray-600'>{user.username ? `@${user.username}` : `Addd a username`}</p>
                    </div>
                    {/* if user is not on others profile that means 
                    he is opening his profile so we will give edit button */}
                     {!profileId && 
                     <button onClick={()=> setShowEdit(true)} className='user-edit flex
                     items-center gap-2 border border-gray-300 hover:bg-gray-50 
                     px-4 py-2 rounded-lg cursor-pointer font-medium transition-colors' >
                        <PenBox className='w-4 h-4'/>
                          Edit
                     </button>} 
                </div>
                <p className='user-bio text-gray-700 text-sm max-w-md'>{user.bio}</p>
                
                <div className='user-location flex flex-wrap items-center gap-x-6 ext-sm text-gray-500 mt-4'>
                ...
                <div className='post-details flex items-center gap-6 mt-6 border-t border-gray-200'>
                    <div>
