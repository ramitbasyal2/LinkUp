...
const StoryModel = ({setShowModal, fetchStories}) => {
  ...
  const bgColors = ["#4f46e5", "#7c3aed", "#db2777","#e11d48","#ca8a04", "#6366F1","#334155"]
  ...
  return (
    ...
    text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
              <div className='arrow_heading text-center flex items-center justify-between'>
                <button onClick={()=> setShowModal(false)} className='text-wwhite p-[20px] cursor-pointer'>
                    <ArrowLeft/>
                </button>
                 <h2 className='text-lg font-semibold'>Create Story</h2>
                 <span className='w-10'></span>
              </div>
              <div className='storyText rounded-lg h-96 flex items-center justify-center 
              relative' style={{background: background}}>
                {
                    mode === 'text' && (
                        <textarea className='bg-transparent text-white w-full h-full
                         text-lg resize-none focus:outline-none' placeholder="What's on your mind?" onChange={(e)=> setText(e.target.value)} value={text} ></textarea>
                    )}
                    {
              ...
              <div className='bgColor flex gap-2'>
                   {bgColors.map((color)=>(
              ...
              <div className='text-btn flex gap-2] '>
                      <button onClick={()=> {setMode('text'); setMedia(null); setPreviewUrl(null)}} 
                      className={`btn flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'text' ? "bg-white text-black" : "bg-zinc-800"}`}>
              ...
              })} className='create-btn w-full flex items-center justify-center gap-2 text-white rounded
